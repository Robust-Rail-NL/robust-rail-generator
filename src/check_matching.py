...
def check_matching(scenario_generator, use_default_material=True, minimal_yard_time=600):
    ...
    if not valid:
        return False
    train_units = [(train, unit, "incoming") for train in scenario_generator.scenario.in_ for unit in (
            train.members)] + [(train, unit, "instanding") for train in scenario_generator.scenario.in_standing for unit in train.members]
    train_unit_requests = [(train, unit, "outgoing") for train in scenario_generator.scenario.out for unit in train.members] + [(train, unit, "outstanding") for train in scenario_generator.scenario.out_standing for unit in train.members]
    ...
    if sorted([unit.type_display_name for _, unit, _ in train_units]) != sorted([unit.type_display_name for _, unit, _ in train_unit_requests]):
    ...
    for in_train, unit, _ in train_units:
        ...
        matching_departures = [(req_unit, out_train.time, typ) for out_train, req_unit, typ in train_unit_requests if req_unit.type_display_name == unit.type_display_name and (out_train.time > in_train.time + sum([t.duration for t in unit.tasks]) or typ == "outstanding")]
        if not matching_departures:
            logging.warning(f"No matching departure found for incoming train unit {unit.id} of type {unit.type_display_name} arriving at {in_train.time} with train {in_train.id}.")
            return False
        ...
        train_unit_requests.remove((next(out_train for out_train, req_unit, typ in train_unit_requests if req_unit.type_display_name == first_departing_train[0].type_display_name and out_train.time == first_departing_train[1] and typ == first_departing_train[2]), first_departing_train[0], first_departing_train[2]))
        logging.info(f"Primitively matched incoming train unit {unit.id} of type {unit.type_display_name} of train {in_train.id} arriving at {in_train.time} with departure at {first_departing_train[1]}.")
