...
class ScenarioGenerator:
    ...
    def create_HIP_scenario(self, source=None):
        """Create the solver format of the scenario file. The default source to use is `self.scenario['in']`, but the source can be specified to use 'self.scenario_in'."""
        # TODO: current solver format does not yet allow in/outstanding, add these later
        logging.info("call create_HIP_scenario()")
        if source is None:
            incoming_trains_scenario = getattr(self.scenario, "in")
        else:
            if source == "self.scenario_in":
                incoming_trains_scenario = self.scenario_in
            else:
                logging.error("Did not understand parameter source", source)
    ...
    def convert_location_to_hip_location(self, file_name):
        ...
        if self.location.ListFields():
            ...
            # Add the possible task types of the location
