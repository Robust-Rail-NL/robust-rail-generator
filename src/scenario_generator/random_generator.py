...
class RandomGenerator:
    ...
    def partitions_with_bounds(self, n, bounds=(1, 3)):
        """Generate all partitions of `n` using only values between bounds."""
        def helper(remaining, current):
            ...
            for i in range(bounds[0], bounds[1] + 1):
                if i <= remaining:
                    yield from helper(remaining - i, current + [i])
        return list(helper(n, []))


    def distribute_units(self, number_trains):
        """
        We have a set of train units, distributed over types: self.train_units has per type a list of units
        These need to be distributed over number_trains trains in total. 
        """
        min_number_trains_per_type = {type_name: math.ceil(len(units) / 3) for type_name, units in self.train_units.items()}
        max_number_train_per_type = {type_name: len(units) for type_name, units in self.train_units.items()}

        min_total = sum(min_number_trains_per_type.values())

        trains_to_divide = number_trains - min_total
        capacities = [max_number_train_per_type[i] - min_number_trains_per_type[i] for i in self.train_units]

        # Generate a random composition of `extra_bins` into `types` parts within `capacities`
        # Use "residual allocation" method
        assignment = {name: 0 for name in self.train_units}
        for i in self.train_units:
            max_extra = min(trains_to_divide, capacities[i])
            assignment[i] = random.randint(0, max_extra)
            trains_to_divide -= assignment[i]

        # Last one takes the rest
        if trains_to_divide > capacities[-1]:
            return None  # overflows last bin's capacity — should never happen if caps are respected
        assignment[-1] = trains_to_divide

        # Add back the minimums
        return [assignment[i] + min_number_trains_per_type[i] for i in self.train_units]
