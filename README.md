...
```bash
protoc --proto_path=protos --python_out=src/py_protobuf \
      protos/*.proto \
      protos/HIP_protos/*.proto
```

To fix the [malfunctioning](^1) absolute imports generated by `protoc`, we use [`protoletariat`](https://github.com/cpcloud/protoletariat):

```bash
protol --create-package --in-place \
       --python-out src/py_protobuf/ \
...
[^1]:
The generated protobuf code won't import cleanly; this is a [known issue](https://github.com/protocolbuffers/protobuf/issues/1491) that won't be fixed by the `protoc` maintainers.

The proposed workaround (which was originally also implemented in this code base) is to [modify the generated code and to programmatically edit the system path](https://github.com/protocolbuffers/protobuf/issues/881#issuecomment-1615919615).
